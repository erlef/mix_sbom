...
defmodule Version.Requirement.Multirange do
  ...
  @spec to_requirement(multi_range()) :: {:ok, Version.Requirement.t()} | :error
  def to_requirement([]), do: Version.parse_requirement("< 0.0.0-0")
  def to_requirement([{:min, :max}]), do: Version.parse_requirement(">= 0.0.0-0")
  ...
  defp range_to_requirement_constraints(
         {{:including, {maj, min, patch, pre, build}}, {:excluding, {maj, next_min, 0, [0], _upper_build}}}
       )
  ...
  defp range_to_requirement_constraints(
         {{:including, {maj, min, 0, [], []}}, {:excluding, {next_maj, 0, 0, [0], _upper_build}}}
       )
  ...
  defp condition_to_multi_range(:~>, {major, minor, nil, pre, _build}),
    do: [{{:including, {major, minor, 0, pre, nil}}, {:excluding, {major + 1, 0, 0, [0], nil}}}]

  defp condition_to_multi_range(:~>, {major, minor, patch, pre, build}),
    do: [{{:including, {major, minor, patch, pre, build}}, {:excluding, {major, minor + 1, 0, [0], nil}}}]
  ...
  defp prefer_more_specific_bound(
    ...
    cond do
      length(List.wrap(build_left)) > length(List.wrap(build_right)) ->
        {inc_left, {maj_left, min_left, patch_left, pre_left, build_left}}

      length(List.wrap(build_right)) > length(List.wrap(build_left)) ->
        {inc_right, {maj_right, min_right, patch_right, pre_right, build_right}}
