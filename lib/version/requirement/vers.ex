...
defmodule Version.Requirement.Vers do
  ...
  ## Examples
      ...
      iex> {:ok, {req, "npm"}} = Version.Requirement.Vers.from_vers("vers:npm/*")
      ...> to_string(req)
      ">= 0.0.0-0"

      iex> {:ok, {req, "elixir"}} =
      ...>   Version.Requirement.Vers.from_vers("vers:elixir/>=1.0.0|<2.0.0-0")
      ...>
      ...> to_string(req)
      "~> 1.0"
  ...
  """
  @spec from_vers(String.t()) :: {:ok, {Version.Requirement.t(), type()}} | :error
  ...
  defp combine_constraints(constraints) do
    with {:ok, parsed_constraints} <- parse_all_constraints(constraints) do
      ...
      |> Enum.map(fn [lower, constraint, upper] ->
        constraint
        |> constraint_to_multi_range()
        |> Multirange.intersect(bounding_range(lower, upper, constraint))
      end)
  ...
  @spec constraint_to_multi_range(atom() | {atom(), Multirange.matchable()}) ::
          Multirange.multi_range()
  defp constraint_to_multi_range(:*), do: [{:min, :max}]

  defp constraint_to_multi_range({:!=, version}), do: [{:min, {:excluding, version}}, {{:excluding, version}, :max}]

  defp constraint_to_multi_range({:<, version}), do: [{:min, {:excluding, version}}]
  defp constraint_to_multi_range({:<=, version}), do: [{:min, {:including, version}}]
  defp constraint_to_multi_range({:>, version}), do: [{{:excluding, version}, :max}]
  defp constraint_to_multi_range({:>=, version}), do: [{{:including, version}, :max}]

  defp constraint_to_multi_range({:==, version}), do: [{{:including, version}, {:including, version}}]

  @spec bounding_range(
          atom() | {atom(), Multirange.matchable()},
          atom() | {atom(), Multirange.matchable()},
          atom() | {atom(), Multirange.matchable()}
        ) :: Multirange.multi_range()
  defp bounding_range(:min, :max, _constraint), do: [{:min, :max}]
  defp bounding_range(:min, {_op, version}, constraint), do: [{:min, bound(version, constraint)}]
  defp bounding_range({_op, version}, :max, constraint), do: [{bound(version, constraint), :max}]

  defp bounding_range({_op1, lower_v}, {_op2, upper_v}, constraint),
    do: [{bound(lower_v, constraint), bound(upper_v, constraint)}]

  @spec bound(Multirange.matchable(), atom() | {atom(), Multirange.matchable()}) ::
          Multirange.bound()
  defp bound(version, {_op, version}), do: {:including, version}
  defp bound(version, _constraint), do: {:excluding, version}
