defmodule SBoM.CycloneDX.V15.License do
  @moduledoc "CycloneDX License model."
  use Protobuf,
    full_name: "cyclonedx.v1_5.License",
    protoc_gen_elixir_version: "0.16.0",
    syntax: :proto3

  oneof(:license, 0)

  field(:id, 1, type: :string, oneof: 0)
  field(:name, 2, type: :string, oneof: 0)
  field(:text, 3, proto3_optional: true, type: SBoM.CycloneDX.V15.AttachedText)
  field(:url, 4, proto3_optional: true, type: :string)
  field(:bom_ref, 5, proto3_optional: true, type: :string, json_name: "bomRef")
  field(:licensing, 6, proto3_optional: true, type: SBoM.CycloneDX.V15.Licensing)
  field(:properties, 7, repeated: true, type: SBoM.CycloneDX.V15.Property)
end
