...
defmodule SBoM.CycloneDX.JSON do
  ...
  @doc false
  @spec encode_json(term(), boolean()) :: binary()
  def encode_json(encodable, pretty)

  # Pretty
  cond do
    Code.ensure_loaded?(:json) and function_exported?(:json, :format, 2) ->
      def encode_json(encodable, true) do
    ...
    Code.ensure_loaded?(Jason) ->
      def encode_json(encodable, true), do: Jason.encode!(encodable, pretty: true)
  ...
  cond do
    Code.ensure_loaded?(JSON) ->
      def encode_json(encodable, false), do: JSON.encode!(encodable)

    Code.ensure_loaded?(Jason) ->
      def encode_json(encodable, false), do: Jason.encode!(encodable)

    true ->
      def encode_json(_encodable, false) do
        Mix.raise("""
  ...
  @doc false
  @spec decode_json(String.t()) :: term()
  def decode_json(json)

  cond do
    Code.ensure_loaded?(JSON) ->
      def decode_json(json), do: JSON.decode!(json)

    Code.ensure_loaded?(Jason) ->
      def decode_json(json), do: Jason.decode!(json)

    true ->
      def decode_json(_json) do
        Mix.raise("""
