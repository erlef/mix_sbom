defmodule SBoM.CycloneDX.V17.Definition.Standard.Requirement do
  @moduledoc "CycloneDX Definition.Standard.Requirement model."
  use Protobuf,
    full_name: "cyclonedx.v1_7.Definition.Standard.Requirement",
    protoc_gen_elixir_version: "0.16.0",
    syntax: :proto3

  field(:bom_ref, 1, proto3_optional: true, type: :string, json_name: "bomRef")
  field(:identifier, 2, proto3_optional: true, type: :string)
  field(:title, 3, proto3_optional: true, type: :string)
  field(:text, 4, proto3_optional: true, type: :string)
  field(:descriptions, 5, repeated: true, type: :string)
  field(:openCre, 6, repeated: true, type: :string)
  field(:parent, 7, proto3_optional: true, type: :string)
  field(:properties, 8, repeated: true, type: SBoM.CycloneDX.V17.Property)
  field(:externalReferences, 9, repeated: true, type: SBoM.CycloneDX.V17.ExternalReference)
end
