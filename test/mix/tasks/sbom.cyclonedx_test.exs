...
defmodule Mix.Tasks.Sbom.CyclonedxTest do
    ...
    Util.in_project(app_path, fn _mix_module ->
      capture_io(:stderr, fn ->
        capture_io(:stdio, fn ->
          ...
          Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-f", "-o", bom_path])
          assert_received {:mix_shell, :info, ["* creating bom.cdx"]}

          Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-o", bom_path])
    ...
    out =
      capture_io(:stdio, fn ->
        Util.in_project(app_path, fn _mix_module ->
          ...
          Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-o", "-"])
        end)
      end)
  ...
  test "schema validation", %{app_path: app_path} do
    Util.in_project(app_path, fn _mix_module ->
      Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-f", "-s", "1.3"])
    ...
    capture_io(:stderr, fn ->
      capture_io(:stdio, fn ->
        Util.in_project(app_path, fn _mix_module ->
          ...
          assert_valid_cyclonedx_bom(bom_path, :json)
          refute File.read!(bom_path) =~ ~s("vulnerabilities")
        end)
      end)
    end)
  end
    ...
    capture_io(:stderr, fn ->
      capture_io(:stdio, fn ->
        Util.in_project(app_path, fn _mix_module ->
          ...
          Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-f", "-r"])
  ...
  describe "filtering with only and targets options" do
    ...
    test "filters dependencies by only option", %{app_path: app_path} do
      Util.in_project(app_path, fn _mix_module ->
        ...
        Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-f", "-o", dev_bom_path, "--only", "dev"])
        ...
        Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-f", "-o", test_bom_path, "--only", "test"])
    ...
    test "filters dependencies by targets option", %{app_path: app_path} do
      Util.in_project(app_path, fn _mix_module ->
        ...
        Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-f", "-o", host_bom_path, "--targets", "host"])
        ...
        Mix.Task.rerun("sbom.cyclonedx", ["-n", "--no-vulnerabilities", "-f", "-o", rpi_bom_path, "--targets", "rpi"])
